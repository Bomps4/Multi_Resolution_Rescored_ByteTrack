...
class Exp(BaseExp):
    ...
    def _get_resizer_normalization(self):
        ...
        if (self.resize_as_tensor):
            ...
            return [T_Resize_as_YOLO(self.test_size,base_value=self.base_value,swap=self.swap),T_To_tensor(self.normalize,self.mean,self.std)]
    ...
    def get_eval_loader(self, batch_size, is_distributed):
        ...
        self.val_dataset = Imagenet_VID_Dataset(self.val_ann_dir,self.val_dat_dir,val=True,transform=transforms,COCO=self.COCO,Add_Background=self.Add_Background,seq_lenght=self.seq_lenght)
