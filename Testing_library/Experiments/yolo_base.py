...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.act = "silu"
        self.mean = torch.tensor([0.485 * 255, 0.456 * 255, 0.406 * 255])[:,None,None]
        self.std = torch.tensor([0.229 * 255, 0.224 * 255, 0.225 * 255])[:,None,None]
        self.normalize = True
        ...
        self.data_dir = "/media/bomps/UB_WIN_SHARED/ILSVRC2015/ILSVRC2015/Data/VID/train/"
        # name of annotation file for training
        self.train_ann_dir = "/media/bomps/UB_WIN_SHARED/ILSVRC2015/ILSVRC2015/Annotations/VID/train/"
        # name of annotation file for evaluation
        self.val_dat_dir = "/media/bomps/UB_WIN_SHARED/ILSVRC2015/ILSVRC2015/Data/VID/val2017"
        ...
        #use multiple sizes (passed to the model could be substituted by checking the format of the input size and evaluation size)
        self.multisize=False
        self.seq_lenght=1
        ...
        self.val_annot_file = '/media/bomps/UB_WIN_SHARED/ILSVRC2015/ILSVRC2015/Data/VID/annotations/IMAGENET_val.json'

        self.bgr = True
        self.lownmsthre = 0.3

        #path to ground truth (havent fused yet with the annotation path in dataloader so for the moment they are divided)
        self.gt_path='/media/bomps/UB_WIN_SHARED/ILSVRC2015/ILSVRC2015/Data/VID/annotations/IMAGENET_val.json'
    ...
    def get_model(self):
        ...
        if getattr(self, "model", None) is None:
            ...
            self.model = YOLOX(self.multisize, self,backbone, head,xyxy=self.xyxy)
    ...
    def get_train_loader(self, batch_size, is_distributed, transforms=[], cache_img=False):
        from ..Dataset.Imagenet import Imagenet_VID_Dataset,Mosaic_Augment,data_dict
        from torch.utils.data import DataLoader
        from ..Dataset.collates import collate_multiresolution,collate_normal
        ...
        dataloader_kwargs = {"num_workers": self.data_num_workers, 'sampler':sampler,'prefetch_factor':4}
        if is_distributed:
            batch_size = batch_size // dist.get_world_size() 
        ...
        if self.multisize:
            #based on collate we change the type of data that will be passed to the network if we have a n
            collate_fn=collate_multiresolution
        else:
    ...
    def _get_resizer_normalization(self,val=False):
        # if(len(self.mean.shape)==1):
        #     self.mean,self.std=self._mean_std_dim()
        from ..My_transforms.Transforms import T_Resize_as_YOLO,T_To_tensor,T_Resize,T_Normalize

        transform=[]

        if (self.resize_as_tensor):
            transform+=[T_To_tensor(swap=self.swap,bgr=self.bgr),T_Resize(self.test_size if val else self.input_size,resize_as_tensor=self.resize_as_tensor)]
        else:
            transform+=[T_Resize_as_YOLO(self.test_size if val else self.input_size,base_value=self.base_value),T_To_tensor(swap=self.swap,bgr=self.bgr)]

        if self.normalize:

            transform+=[T_Normalize(self.mean,self.std)]
    ...
    def get_optimizer(self, batch_size,is_distributed):
    ...
    def get_eval_loader(self, batch_size, is_distributed,for_train=False):
        from ..Dataset.Imagenet import Imagenet_VID_Dataset,NAMES,Coco_Dataset
        from ..Dataset.collates import collate_multiresolution,collate_normal
        
        transforms=self._get_resizer_normalization(val=True)
        self.val_dataset = Coco_Dataset(self.val_dat_dir,self.val_annot_file,transforms,None,True)
        ...
        if is_distributed:
            ...
            if for_train:
                sampler = torch.utils.data.RandomSampler(self.val_dataset)
            else:
                sampler = torch.utils.data.SequentialSampler(self.val_dataset)
        
        if isinstance(self.test_size,list):
            #based on collate we change the type of data that will be passed to the network if we have a n
            collate_fn=collate_multiresolution
        else:
            collate_fn=collate_normal
    ...
    def get_evaluator(self,saving_dir):
        ...
        data_dict_current = data_dict[self.COCO]
        imgvid_dict = data_dict['Imgvid']

        self.selected_classes=torch.tensor([ data_dict[self.COCO][i] - int(not self.Add_Background) for i in data_dict[self.COCO]])

        print(self.selected_classes)

        self.from_coco_to_imgvid = {(data_dict_current[i]- int(not self.Add_Background)):imgvid_dict[i] for i in data_dict_current}
