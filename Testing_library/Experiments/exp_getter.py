...
def get_exp_by_file(exp_file):
    try:
        print('directory path',os.path.dirname(exp_file)+'/')
        print(sys.path)
        sys.path.append(os.path.dirname(exp_file))
        print(sys.path)
        print(__package__)
        print(os.path.basename(exp_file).split(".")[0])
        current_exp = importlib.import_module('Testing_library.Experiments.'+os.path.basename(exp_file).split(".")[0])
        print('current module',current_exp)

        exp = current_exp.Exp()
        #exp.__module__=current_exp
        print('sono tipe',type(exp))
    except Exception:
