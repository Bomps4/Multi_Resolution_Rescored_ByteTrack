...
def multiclass_nms(
    ...
    return dets, labels[keep][:,None],scores[keep][:,None]
...
class NanoDetPlus(nn.Module):
    ...
    def post(self,prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=False):
        ...
        for i in range(scores.shape[0]):
            ...
            results = multiclass_nms(
                ...
                score,
                score_thr=conf_thre,
                nms_cfg=dict(type="nms", iou_threshold=nms_thre,class_agnostic=class_agnostic),
                max_num=100,
                )
            ...
            if(results[0].shape[0]==0):
                ...
                result_list.append(torch.cat(results,dim=-1))
    ...
    def forward(self, x,target=None):
        ...
        if not torch.onnx.is_in_onnx_export():
            ...
            [self.num_classes, 4 * (self.head.reg_max + 1)], dim=-1
        )
            
            x=self.head.get_bboxes(cls_scores, bbox_preds,img_shape)
    ...
    def forward_train(self, gt_meta):
        ...
        loss = self.head.loss(head_out, gt_meta, aux_preds=aux_head_out)
        return head_out, loss