...
class Coco_Dataset(torchvision.datasets.CocoDetection):
	def __init__(self, img_folder, ann_file, transforms,data_dependent_transforms,val = False):
		
		super(Coco_Dataset, self).__init__(img_folder, ann_file)

		if len(transforms) != 0:
			self._transforms = Selective_Compose(transforms)
		else:
			self._transforms = None
		
		# if len(data_dependent_transforms) != 0:
		# 	self._data_dependent_transforms = Data_Dependent_aumentations(self,data_dependent_transforms)
		# else:
		self._data_dependent_transforms = None
	...
	def add_transform(self,transform):
		self._transforms.add_trasform(transform)

	def remove_transform(self,idx):
		self._transforms.remove_transform(idx)

	# def _load_image(self, id: int) -> Image.Image:
	# 	path = self.coco.loadImgs(id)[0]["file_name"]
	# 	array_image = cv2.imread(os.path.join(self.root, path))
	...
	def pull(self,idx):
		...
		if len(target)!=0:
			...
			new_target = {i:[] for i in target[0] if (i !='segmentation' and i!= 'bbox' and i!= 'category_id')}
				...
				for key in instance:
					...
					if key == 'bbox':
						if len(instance[key])==0:
							new_target['boxes'].append(torch.zeros(0,4))
						else:
							new_target['boxes'].append(box_xywh_to_xyxy(torch.tensor(instance[key]).float()))
					elif key == 'category_id':

						new_target['labels'].append(torch.tensor(self.class_ids.index(instance[key])))
					else:
						new_target[key].append(torch.tensor(instance[key]))



			new_target = {i:torch.stack(new_target[i],dim=0) for i in new_target}
			new_target.pop('image_id')
			
			

			target = {'images_id': torch.tensor([image_id]), **new_target,"height":torch.tensor([height]),'width':torch.tensor([width])}
		
		else:
			target = {'images_id': torch.tensor([image_id]),'id':torch.zeros(0), 'labels':torch.zeros(0),'boxes':torch.zeros(0,4),"height":torch.tensor([height]),'width':torch.tensor([width]),'area':torch.zeros(0),'iscrowd':torch.zeros(0)}
		
		return img, target

	def __getitem__(self, idx):

		img, target = self.pull(idx)
			
		if self._data_dependent_transforms is not None:
			img, target = self._data_dependent_transforms(img,target)
...
class Imagenet_VID_Dataset(Dataset):
	def __init__(self,imagenet_annotation:str,imagenet_images:str,transform:Union[List[nn.Module],nn.Module],val=False,for_train=True,seq_lenght=1,sub_camp=1,max_labels=50,COCO='Imgvid',Add_Background=True):
		...
		for indice,i in enumerate(dirs):
			...
			for j in videos:
				to_return.append([]) #in to return è pieno di liste ogni lista è un video 
				...
				for f,img in zip(frames,images):
					...
					if((for_train or not val) and not changed_once ):
						continue
					if(len(obj["labels"])==0):
						obj["labels"]=torch.empty((0,)).long()
						obj["boxes"]=torch.empty((0,4))
					else:
						obj["labels"]=torch.tensor(obj["labels"]).to(torch.int64)
	...
	def __getitem__(self, idx:int)->Tuple[List[Tensor],Dict[str,Union[Tensor,int,float]]]:
		...
		# properties of the image passed 
		filename=datum['file_name']
		width=torch.tensor([datum["width"]])
		height=torch.tensor([datum['height']])
		image_id=torch.tensor([datum['image_id']])

		#index video 
		vid_index=torch.tensor([datum['vid_index']])



		annotations= datum['annotations']
		
		image,gts=self.transform(read_image(filename),{**annotations,'vid_index':vid_index,'width':width,'height':height,'images_id':image_id})
		...
		'''
		#gts={"width":i["width"],"height":i["height"],"image_id":i["image_id"],'file_name':i['file_name'],'vid_index':i['vid_index']}
